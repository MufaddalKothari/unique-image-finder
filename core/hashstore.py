...
class HashStore:
    ...
    def get(self, path: str, mtime: int, size: int, hash_size: int):
        ...
        if not row:
            return None
        hexstr = row[0]
        # convert hex string to ImageHash instance
        try:
    ...
    def set(self, path: str, hash_obj, hash_size: int):
        """
        Store or update hash for path. hash_obj should be imagehash.ImageHash (or str).
        """
        if hash_obj is None:
            return
        hexstr = str(hash_obj)
    ...
    def bulk_get(self, paths, hash_size: int) -> Dict[str, object]:
        """
        Given a list of absolute paths, return a dict {path: ImageHash} for entries present and valid.
        """
        out = {}
        cur = self._conn.cursor()
        for p in paths:
    ...
    def remove_missing(self, existing_paths):
        """
        Remove DB rows for files that no longer exist. existing_paths is a set/list of current absolute paths.
        """
        cur = self._conn.cursor()
